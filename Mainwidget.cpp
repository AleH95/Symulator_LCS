    ...
    ui(new Ui::Widget)
{
    ...
    int tmpsem[4] = {2,3,4,5};
    ZwUpDw = new Stos(UZW+DZW,tmpzw);
    Semaforki = new Stos(4,tmpsem);
    ...
    MainLab->setStyleSheet("background-color:green");
    MainLab->setGeometry(QRect(0,0,50,50));
    Lab2->setText("Ala");
    Lab2->setStyleSheet("background-color:red");
    Lab2->setGeometry(QRect(0,0,50,25));
    Lab3->setText("Kot");
    Lab3->setStyleSheet("background-color:orange");
    Lab3->setGeometry(QRect(0,25,50,25));

    Semafor* semaforek = new Semafor();
    ...
    //connect(GenTrain,SIGNAL(clicked(bool)),this,SLOT(SemaforChange()));
...
void Widget::inizializza()
{
    ...
    for(j=0;j<ALTEZZA*LUNGHEZZA;j++)
    {
        ...
        semafory[j] = new Semafor();
        QString ela = "SEM"+QString::number(j/(LUNGHEZZA))+QString::number(i);
        if(inUp(j,Semaforki)==true)
        {
            //alla[j]->
        }
    }
    ...
    for(j=0;j<ASEMAFORY*LSEMAFORY;j++)
    {
            ...
            connect(Semafory[j], SIGNAL(clicked(bool)), SsignalMapper, SLOT(SemaforChange())); //polaczenie signalu zwrotnicy do signalmapper
            //connect(Semafory[j], SIGNAL(clicked()), SsignalMapper, SLOT(setLight(1)));
            //Needed to pass the value in order to distingush the coords at which the semafor is in the visualization
            int temp2 = Semaforki->GetThat(j);
            SsignalMapper->setMapping(Semafory[j], temp2); //nastawianie parametru wedlug ktorego signalmapper rozpoznaje wcisniety przycisk
            i++;
            if(i>=LSEMAFORY)
            {
                i=0;
...
void Widget::SemaforChange()
{
    temptor=alla[2];
    //if(temptor->getLight()==0)
    //{
        temptor->setStyleSheet("background-color: orange");
    //}
}
