...
void Widget::inizializza()
{
    ...
    for(j=0;j<RAILPGZ;j++)
    {
        ...
        QString ela = "Z"+QString::number((j/LZWROTNICE)+((RAILLDZ+RAILLGZ)/LZWROTNICE))+QString::number(i);
            ...
            if(TrasaPociag(n,trainnr,WHITE))
            {
                for(z=0;z<Pociag::getQuanto();z++)
                {
                    if((peron=IsIn(Ciuf[z]->getCoords(),Perony,ALTEZZA))!=-1)
                    {
                        ...
                        Trasa(Perony[peron]+1,'w',alla[Perony[peron]]->getITrain());
                    }
        ...
        else if((Ciuf[trainnr]->getDirection())==2)
        {
            ...
            for(z=0;z<Pociag::getQuanto();z++)
            {
                if((peron=IsIn(Ciuf[z]->getCoords(),Perony,ALTEZZA))!=-1)
                {
                    ...
                    cleanAdjacent(numbers,GREY);
                    qDebug("Reverse Trasa");
                    ReverseTrasa(Perony[peron]-1,'w',alla[Perony[peron]]->getITrain());
                }
...
void Widget::ObslugaPeronu(int n)
{
    ...
    if(alla[n]->getITrain())
    {
        ...
        if((Ciuf[trainnr]->getDirection())==1) //==1 w prawo, ==2 w lewo
        {
            ...
            //Podajemy z aktualnym bloczku znajdywania sie pociagu!!
            if(TrasaPociag(n+1,trainnr,WHITE))
            {
        ...
        else if((Ciuf[trainnr]->getDirection())==2)
        {
            ...
            qDebug("Reverse Trasa");
            if(ReverseTrasaPociag(n-1,trainnr,WHITE))
            {
                if((peron=IsIn(Ciuf[trainnr]->getCoords(),End,ALTEZZA))!=-1)
                {
                    Pociag::decrement();
