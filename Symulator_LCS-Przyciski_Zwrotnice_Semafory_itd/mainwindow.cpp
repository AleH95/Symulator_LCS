    ...
    ,ui(new Ui::MainWindow)
{
    ...
    for (int j = 0; j < TORY_HEIGHT; j++){
        ...
        for (int i = 0; i < TORY_WIDTH; i++)
        {
            ...
            butt_tory[j][i]->setFixedSize(53,26);
...
*/
   ...
   // connect(butt_potw,SIGNAL(clicked(bool)),this,SLOT(Set_route()));
...
void MainWindow::Set_route(int variable)
{
switch (variable)
{
    case 0:
    ...
    for (int j = 3; j <= 11; j++)
    {
        for (int i = 0; i <= 14; i++)
        {
            if ((j==3||j==6||j==8||j==11)&&(i<3))
                butt_tory[j][i]->setStyleSheet("background-color:rgb(255,255,255)");
            else if ((i==0||i==2)&&j!=7)
                butt_tory[j][i]->setStyleSheet("background-color:rgb(255,255,255)");
            if ((j==5||j==9)&&i>2&&i<14)
                butt_tory[j][i]->setStyleSheet("background-color:rgb(255,255,255)");
